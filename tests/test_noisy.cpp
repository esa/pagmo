...
int test_noisy(const std::vector<problem::base_ptr> & probs, double noise_mean, double noise_stddev, int num_trials, double tol)
{
	...
	for(unsigned int i = 0; i < probs.size(); i++){
		...
		pagmo::problem::noisy prob_noisy(*(probs[i]), noise_mean, noise_stddev, i*177 + 23);
...
{	
	...
	return test_noisy(probs, 0.0, 0.1, 100000, 0.001) ||
		   test_noisy(probs, 1.0, 0.1, 100000, 0.001) ||
		   test_noisy(probs, -1.0, 0.2, 100000, 0.05) ||
		   test_noisy(probs, 3.14, 1.0, 100000, 0.01) || // Needs more trials for better tolerance...
		   test_noisy(probs, 0.0, 0.0, 100000, 0.001);
}
