...
int main()
{
	...
	for(unsigned int ch=0; ch < 3; ch++) {
		...
		std::ofstream myfile;
		myfile.open((std::string("pagmo_constraints_") + get_constrained_name(ch) + "_island.tex").c_str());
		...
		for (unsigned int pr=0; pr<probs.size();++pr) {
			...
			for (unsigned int al=0; al<algos.size(); ++al) {
				...
				std::vector<double> feasible_champions_f;

				for (size_t i=0; i<number_of_islands; ++i) {
					island myisland(*constrained_algorithm, *constrained_problem, number_of_individuals);
					myisland.evolve(number_of_migrations);
					myisland.join();

					// compute mean
					if(probs[pr]->feasibility_x(myisland.get_population().champion().x)) {
						feasible_champions_f.push_back(myisland.get_population().champion().f[0]);
					}

					// store all the individuals in a vector that will be used to sort the population and so on
						const population &current_population = myisland.get_population();
				...
				// mean chanpion computation
				double mean_among_feas_champions = 0.;

				for(std::vector<double>::size_type i=0; i<feasible_champions_f.size(); i++) {
					mean_among_feas_champions += feasible_champions_f.at(i);
				}
				if(feasible_champions_f.size() != 0)
					mean_among_feas_champions /= feasible_champions_f.size();

				// compute std dev
				double std_dev_among_feas_champions = 0.;

				for(std::vector<double>::size_type i=0; i<feasible_champions_f.size(); i++) {
					std_dev_among_feas_champions += pow((feasible_champions_f.at(i) - mean_among_feas_champions),2);
				}

				if(feasible_champions_f.size() != 0)
					std_dev_among_feas_champions = sqrt(std_dev_among_feas_champions / feasible_champions_f.size());
