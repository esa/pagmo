...
int main()
{
	...
	algorithm::bee_colony algo1(100);
	algorithm::bee_colony algo2(100*number_of_islands/2);
	...
	for (unsigned int pr=0; pr<prob.size();++pr) {
		...
		for (unsigned int al =0; al<algo.size(); ++al) {
			const std::string algo_name = ((al==1) ? std::string("Sequential " + algo[al]->get_name()) : algo[al]->get_name());
			std::cout << algo_name << '\n' << '\n';
			...
			for (unsigned int to=0; to<topo.size(); ++to) {
				archipelago a;
				if(al == 1) {//sequential
					a = pagmo::archipelago(topo1); //use unconnected topology
					a.push_back(island(*prob[pr],*algo[al],number_of_individuals)); //push just one island
