...
algorithm::base_ptr get_constrained_algo(algorithm::base_ptr algo, algorithm::base_ptr algo_immune, int i) {
	switch(i) {
	case(0):
		return algorithm::cstrs_immune_system(*algo,*algo_immune, 100,
											  algorithm::cstrs_immune_system::INFEASIBILITY,
											  algorithm::cstrs_immune_system::CHAMPION).clone();
		break;
	case(1):
		return algorithm::cstrs_immune_system(*algo,*algo_immune, 5000,
											  algorithm::cstrs_immune_system::INFEASIBILITY,
											  algorithm::cstrs_immune_system::CHAMPION).clone();
		break;
	case(2):
		return algorithm::cstrs_immune_system(*algo,*algo_immune, 20000,
											  algorithm::cstrs_immune_system::INFEASIBILITY,
											  algorithm::cstrs_immune_system::CHAMPION).clone();
		break;
	default:
...
std::string get_constrained_name(int ch) {
	switch(ch) {
	case(0):
		return "cstrs_immune_system_gen_100";
		break;
	case(1):
		return "cstrs_immune_system_gen_5000";
		break;
	case(2):
		return "cstrs_immune_system_gen_20000";
		break;
	default:
...
int main()
{
	...
	std::vector<algorithm::base_ptr> algos_immune;
...
//											   algorithm::sga_gray::crossover::SINGLE_POINT).clone());
	...
	algos_immune.push_back(algorithm::de(70, 0.8, 0.9, 2, 1e-15, 1e-15).clone());
		...
		for (unsigned int to=0; to<topo.size(); ++to) {
			...
			for (unsigned int pr=0; pr<probs.size();++pr) {
				...
				for (unsigned int al=0; al<algos.size(); ++al) {
					...
					algorithm::base_ptr constrained_algorithm = get_constrained_algo(algos[al], algos_immune[al],ch);
