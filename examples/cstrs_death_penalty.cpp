...
algorithm::base_ptr get_constrained_algo(algorithm::base_ptr algo, int i) {
	switch(i) {
	case(0):
		return algorithm::de(100, 0.8, 0.9, 2, 1e-15, 1e-15).clone();
		break;
	case(1):
		return algorithm::de(5000, 0.8, 0.9, 2, 1e-15, 1e-15).clone();
		break;
	case(2):
		return algorithm::de(20000, 0.8, 0.9, 2, 1e-15, 1e-15).clone();
		break;
	case(3):
		return algorithm::de(100, 0.8, 0.9, 2, 1e-15, 1e-15).clone();
		break;
...
std::string get_constrained_name(int ch) {
	switch(ch) {
	case(0):
		return "cstrs_death_penalty_simple_100";
		break;
	case(1):
		return "cstrs_death_penalty_simple_5000";
		break;
	case(2):
		return "cstrs_death_penalty_simple_20000";
		break;
	case(3):
		return "cstrs_death_penalty_kuri_100";
		break;
	case(4):
		return "cstrs_death_penalty_kuri_5000";
		break;
	case(5):
		return "cstrs_death_penalty_kuri_20000";
		break;
	default:
...
std::string undersdeath_penalty_to_space(std::string text) {
				...
				for (unsigned int al=0; al<algos.size(); ++al) {
					...
					for(population::size_type i=0; i<(final_population_size-1); i++) {
						for(population::size_type j=i+1; j<final_population_size; ++j) {
							if(probs[pr]->compare_x(final_population.at(j).cur_x,
													final_population.at(i).cur_x) ) {
					...
					int viol_median = number_violated_constraints(probs[pr]->compute_constraints(final_population.at(median).cur_x), probs[pr]);
					// number of violated constraints at the best solution:
					int viol_best = number_violated_constraints(probs[pr]->compute_constraints(final_population.at(0).cur_x), probs[pr]);
					// number of violated constraints at the worst solution:
					int viol_worst = number_violated_constraints(probs[pr]->compute_constraints(final_population.at(final_population_size-1).cur_x), probs[pr]);

					double mean_viol_median = mean_violated_constraints(probs[pr]->compute_constraints(final_population.at(median).cur_x), probs[pr]);
