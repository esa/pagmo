...
#ifndef PAGMO_PROBLEM_PYTHON_BASE_STOCHASTIC_H
#define PAGMO_PROBLEM_PYTHON_BASE_STOCHASTIC_H
...
#include "../../src/problem/base_stochastic.h"
...
void save_construct_data(Archive &, const pagmo::problem::python_base_stochastic *, const unsigned int);

template <class Archive>
inline void load_construct_data(Archive &, pagmo::problem::python_base_stochastic *, const unsigned int);

}}

namespace pagmo { namespace problem {

class __PAGMO_VISIBLE python_base_stochastic: public base_stochastic, public boost::python::wrapper<base_stochastic>
{
	public:
		...
		{
			return this->base_stochastic::get_name();
		}
		...
		{
			return this->base_stochastic::human_readable_extra();
		}
		...
		bool py_equality_operator_extra(const base &p) const
		{
			...
			return base_stochastic::equality_operator_extra(p);
		}
	...
	protected:
		...
		bool equality_operator_extra(const base &p) const
		{
			...
			if (get_typename() != dynamic_cast<const python_base_stochastic &>(p).get_typename()) {
				return false;
			}
		...
		void compute_constraints_impl(constraint_vector &c, const decision_vector &x) const
		{
			if (this->get_override("_compute_constraints_impl")) {
				...
				base_stochastic::compute_constraints_impl(c,x);
			}
        }
	...
	private:
		...
		void serialize(Archive &ar, const unsigned int)
		{
			...
			ar & boost::serialization::base_object<boost::python::wrapper<base_stochastic> >(*this);
		}
};
...
template <class Archive>
inline void save_construct_data(Archive &ar, const pagmo::problem::python_base_stochastic *prob, const unsigned int)
{
...
template <class Archive>
inline void load_construct_data(Archive &ar, pagmo::problem::python_base_stochastic *prob, const unsigned int)
{
	// Retrieve data from archive required to construct new instance.
	int dimension;
	ar >> dimension;
	// Invoke inplace constructor to initialize instance of the island.
	::new(prob)pagmo::problem::python_base_stochastic(dimension);
}
