...
class scipy_fmin(_scipy_base):
	...
	def evolve(self,pop):
		...
		retval = self.solver(lambda x: prob.objfun(concatenate((x,x0_comb)))[0],x0,xtol=self.xtol, ftol=self.ftol, maxiter=self.maxiter, maxfun=self.maxfun, full_output=self.full_output, disp=self.disp, retall=self.retall)
	...
	def human_readable_extra(self):
		return "maxiter = " + str(self.maxiter) + ", xtol = " + str(self.xtol) + ", ftol = " + str(self.ftol) + ", maxfun = " +str(self.maxfun)
...
class scipy_l_bfgs_b(_scipy_base):
	...
	def evolve(self,pop):
		...
		if self.screen_output:
			...
			iprn = -1
		retval = self.solver(lambda x: array(prob.objfun(concatenate((x,x0_comb))),dtype=float),x0,bounds = prob_bounds,approx_grad = True, iprint = iprn, pgtol = self.pgtol, maxfun = self.maxfun, factr = self.factr, m = self.m, epsilon=self.epsilon)
	...
	def human_readable_extra(self):
		return "maxfun = " + str(self.maxfun) + ", m = " + str(self.m) + ", factr = " + str(self.factr) + ", pgtol = " +str(self.pgtol) + ", epsilon = " +str(self.epsilon)
