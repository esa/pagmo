...
#ifndef PAGMO_CONST_OUT_RATE_MIGRATION_SCHEME_H
#define PAGMO_CONST_OUT_RATE_MIGRATION_SCHEME_H
...
/// Migration scheme with constant outgoing migration rate.
/**
 * This is the migration model in which the migration is "destination driven".
 */
class __PAGMO_VISIBLE ConstOutRateMigrationScheme: public MigrationScheme
{
	public:
		/// Constructor.
		/**
		 ...
		 */
		ConstOutRateMigrationScheme(const base_topology& _topology, uint32_t seed = static_rng_uint32()())
		 ...
		 */
		ConstOutRateMigrationScheme(const ConstOutRateMigrationScheme& ms):MigrationScheme(ms),rng(ms.rng) { }
		
		/// Virtual destructor.
		virtual ~ConstOutRateMigrationScheme() { }; 
		...
		virtual MigrationScheme* clone() const { return new ConstOutRateMigrationScheme(*this); }
