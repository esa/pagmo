...
de_1220::de_1220(int gen, int variant_adptv, const std::vector<int> & allowed_variants, bool memory, double ftol, double xtol):base(), m_gen(gen),
	...
	if (variant_adptv < 1 || variant_adptv > 2) {
		pagmo_throw(value_error,"the index describing the adaptive variant must be one of 1 ... 2");
	}
	for (unsigned int i = 0; i < allowed_variants.size(); ++i) {
		if ( (allowed_variants[i] <1) || (allowed_variants[i]>18) ) {
			pagmo_throw(value_error,"allowed variants must all be n [1,18]");
		}
	}
}

/// Clone method.
base_ptr de_1220::clone() const
{
	return base_ptr(new de_1220(*this));
}

/// Evolve implementation.
/**
 * Run the DE-1220 algorithm for the number of generations specified in the constructors.
 * At each improvments velocity is also updated.
 *
 * @param[in,out] pop input/output pagmo::population to be evolved.
 */

void de_1220::evolve(population &pop) const
{
	...
	for (int gen = 0; gen < m_gen; ++gen) {
		...
		for (size_t i = 0; i < NP; ++i) {
			...
			if (VARIANT == 1) { /* strategy DE0 (not in our paper) */
			...
			/*-------as a first guess.---------------------------------------------------------------*/
			else if (VARIANT == 2) { /* strategy DE1 in the techreport */
				if (m_variant_adptv==2) {
			...
			else if (VARIANT == 3) { /* similiar to DE2 but generally better */
				if (m_variant_adptv==2) {
...
/// Algorithm name
std::string de_1220::get_name() const
{
	return "DE - 1220";
}
 ...
 */
std::string de_1220::human_readable_extra() const
{
	std::ostringstream s;
	s << "gen:" << m_gen << ' ';
