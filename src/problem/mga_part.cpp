...
mga_part::mga_part(const std::vector<kep_toolbox::planet_ptr> seq, 
			 ...
			 const kep_toolbox::array3D v_inf_in
			 ): 
			 base(4*(seq.size()-1)), m_tof(tof), m_t0(t0), m_v_inf_in(v_inf_in)
{
...
mga_part::mga_part(const mga_part &p) : base(p.get_dimension()), m_tof(p.m_tof), m_t0(p.m_t0), m_v_inf_in(p.m_v_inf_in)
{
...
try {
	...
	kep_toolbox::array3D v_out,r;
	kep_toolbox::array3D v_beg_l,v_end_l;
	kep_toolbox::sum(v_end_l, m_v_inf_in, v_P[0]);
	for (size_t i = 0; i<(m_seq.size()-1); ++i) {
		// Fly-by
...
std::string mga_part::human_readable_extra() const
{
	...
	oss << "\n\tStarting epoch: " << m_t0;
	oss << "\n\tStarting Vinf in: " << m_v_inf_in << std::endl;
	return oss.str();
}
