 ...
 */
dtlz2::dtlz2(int k, fitness_vector::size_type fdim):base_dtlz(k + fdim - 1, fdim)
{
	// Set bounds.
	set_lb(0.0);
	set_ub(1.0);
...
/// Clone method.
base_ptr dtlz2::clone() const
{
	return base_ptr(new dtlz2(*this));
}

/// Implementation of the distance function g
double dtlz2::g_func(const decision_vector &x) const
{
	double y = 0.0;
...
void dtlz2::objfun_impl(fitness_vector &f, const decision_vector &x) const
{
...
std::string dtlz2::get_name() const
{
	return "DTLZ2";
}
}}

BOOST_CLASS_EXPORT_IMPLEMENT(pagmo::problem::dtlz2);
