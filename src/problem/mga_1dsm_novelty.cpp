 ...
 */
mga_1dsm_novelty::mga_1dsm_novelty(const std::vector<kep_toolbox::planet_ptr> seq, 
...
//			 base( 6 +  (int)(seq.size()-2) * 4, 0, int(mo) + 1,0,0,0.0), m_n_legs(seq.size()-1), m_add_vinf(add_vinf)
			 // for convenience, we switched the multiobjective option here in favor of experimenting with novelty search. Novelty
			 // measure is based on different point of space and time where the spacecraft is ending up.
			 base( 6 +  (int)(seq.size()-2) * 4, 0, 4,0,0,0.0), m_n_legs(seq.size()-1), m_add_vinf(add_vinf)
...
mga_1dsm_novelty::mga_1dsm_novelty(const mga_1dsm_novelty &p) : base(p.get_dimension(), 0, p.get_f_dimension(),0,0,0.0), m_n_legs(p.m_n_legs), m_add_vinf(p.m_add_vinf)
{
...
/// Clone method.
base_ptr mga_1dsm_novelty::clone() const
{
	return base_ptr(new mga_1dsm_novelty(*this));
}

/// Implementation of the objective function.
void mga_1dsm_novelty::objfun_impl(fitness_vector &f, const decision_vector &x) const
{
...
// NOVELTY MEASURE
	...
	f[1] = r_P[m_n_legs][1];
	f[2] = r_P[m_n_legs][2];
	f[3] = std::accumulate(T.begin(),T.end(),0.0);
...
std::string mga_1dsm_novelty::pretty(const std::vector<double> &x) const {
...
std::string mga_1dsm_novelty::get_name() const
{
	return "MGA-1DSM (novelty version)";
}
 ...
 */
void mga_1dsm_novelty::set_tof(double tl, double tu) {
 ...
 */
void mga_1dsm_novelty::set_launch_window(const kep_toolbox::epoch& start, const kep_toolbox::epoch& end) {
 ...
 */
void mga_1dsm_novelty::set_vinf(const double vinf) {
	set_ub(3,vinf*1000);
}
 ...
 */
std::vector<kep_toolbox::planet_ptr> mga_1dsm_novelty::get_sequence() const {
	return m_seq;
}
 ...
 */
std::string mga_1dsm_novelty::human_readable_extra() const
{
...
BOOST_CLASS_EXPORT_IMPLEMENT(pagmo::problem::mga_1dsm_novelty);
